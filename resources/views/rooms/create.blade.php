<x-layout title="Create Room">
    <div>
        <div class="mb-8 flex items-center gap-4">
            ...
            <div>
                <h1 class="text-foreground text-3xl font-bold">Create New Room</h1>
                <p class="text-muted-foreground mt-1 text-sm">Add a new room to your property</p>
            </div>
        </div>
        ...
        <div class="border-border bg-card mx-auto rounded-xl border p-8">
            ...
            <form method="POST" action="{{ route('rooms.store') }}" class="space-y-6">
                ...
                <div class="grid gap-2">
                    ...
                    <input type="number" id="floor" name="floor" min="0" value="{{ old('floor') }}"
                ...
                <div class="grid gap-2">
                    ...
                    <input type="number" id="monthly_rent" name="monthly_rent" min="0" step="0.01"
                        value="{{ old('monthly_rent') }}" placeholder="e.g., 5000.00"
                        class="border-input bg-background text-foreground placeholder:text-muted-foreground
                               focus-visible:border-ring focus-visible:ring-ring/50
                               flex h-9 w-full rounded-md border px-3 py-1 text-sm 
                               transition-colors focus-visible:outline-none focus-visible:ring-[3px]
                    ...
                    <div class="relative">
                        <select id="status" name="status"
                            ...
                            <option value="Occupied"
                                {{ old('status') === 'Occupied' ? 'selected' : '' }}>Occupied</option>
                            <option value="Under Maintenance"
                                {{ old('status') === 'Under Maintenance' ? 'selected' : '' }}>Under
                                Maintenance</option>
                        </select>
